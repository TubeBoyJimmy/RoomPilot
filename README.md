...
原始碼版本與保存來源請見 [HISTORY.md](HISTORY.md)；`v0.1.0`～`v0.5.0` 標籤可用於比較歷次程式演進。

RoomPilot 是在 Windows 本機使用的 REW 量測與 PEQ 專案工具。建立空間、直接匯入 `.mdat`、確認 Mic Cal 與量測品質，再產生可手動套用的 PEQ 建議，最後用補錄驗證結果。

**一般 `.mdat V2` 匯入使用內建解析器，不需要開啟或安裝 REW，也不會在背景啟動 REW。** REW 用來錄製量測；RoomPilot 負責讀取、分析與保存專案。v0.5 的支援範圍與驗證限制請見 [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md)。

## v0.5：分開檢查削峰收益與修正代價

- 保留每版三策略、自動保存與標籤切換。Q 範圍仍由使用者依 DSP 設定，不以降低 Q 上限或填滿 Band 作為改善方法。
- 補上原本高於目標、削完後變成低處的代價；每筆原始 A/B 與偏移量測分開驗算。舊平均指標與新逐筆保護分開標示。
- 進階設定新增共同上限：全頻點新增缺口 RMS **1 dB**、單點新增缺口 **3 dB**、滑動 1/3 octave 平均削減 **4 dB**、固定 80–200 Hz 平均削減 **3 dB**、校正範圍外最大減益 **3 dB**。這些是可調的工程起點，並非經聆聽驗證的門檻。
- 依峰中心、兩側寬度與原始量測超出量，共同比較窄／寬、深／淺起點，再精修 Hz／Gain／Q。以完整合成曲線驗算，不把 Q 撞上限當成房間模態辨識。
- 目前選取的相同 Baseline 方案，若在相同目標與新限制下仍可行，會帶入搜尋並保留較佳完整結果。另顯示固定低頻走勢差異，不自動修改目標或推定偏好。
- 近重複濾波器嘗試完整曲線擬合合併，取整後誤差與限制均通過才接受。單段深度不足時保留串接並說明。
- 新增缺口依實際量測／校正範圍檢查；完整 PEQ 的局部平均與範圍外減益則另在 1 Hz–Nyquist 模型曲線檢查，避免把削減代價推到邊界外。這只驗算 EQ，不會把範圍外未量聲學效果當成已知。擴到 500 Hz 若仍有新限制衝突，會列出頻段、實際值與上限；可明確調整限制或選整段重新最佳化。
- 求解、手動驗算與繪圖共用穩定的 RBJ 幅頻公式；已保存的舊 PEQ 不會因更新而重新計算。

首次比較可沿用 **30–200 Hz、相同 Band／Q／Gain 能力、深入搜尋與相同目標**，保留新增保護預設，查看三組完整曲線與診斷。要實驗 500 Hz 可選整段重新最佳化；需要精確延續某版時才用保留擴充。預測仍需完整替換套用與補錄確認。

完整規則見 [MODEL.md](MODEL.md)，測試與 Baseline 比較見 [VALIDATION.md](VALIDATION.md)。

## v0.4：一個版本，三種可切換策略

- 按「產生 PEQ」後，自動保存一個版本，例如 PEQ v7，內含「保護低處／平衡削峰／充分削峰」三個標籤。直接切換完整曲線、參數與依據，不需另外預覽或儲存。不同容許量可能得到相同參數，三個策略紀錄仍保留。
- 三組採同一個主目標：直接降低殘留波峰 RMS。低處額外減益在求解過程中逐位置限制，分別使用你輸入容許量的 35%、65%、100%；預設總容許量 1 dB RMS。這些比例與上限是產品設定，不是聲學標準，也不是單一頻點的最大減益。
- 精修後再次嘗試新增 Band，探索各殘留峰、對數頻率起點與同時配置多段的另一條路徑。Band 數是上限，不再用每段固定成本抵銷削峰收益；主目標近乎相同時才偏好較小幅度或較少段數。
- 軟體 DSP 可指定每聲道 1–128 段。128 是本版的運算資源上限，不是設備規格；可用更多段不表示必須填滿。計算可取消，取消或失敗不會保存半個版本。
- 「沿用已選 PEQ，僅擴充頻段」保留當前策略的確切參數與目標。「先完成 200 Hz 以下」則是新計算時的中心配置順序，從一開始就以完整指定頻段計算代價。兩者用途不同；要精確延續某個 200 Hz 版本，請選前者。
- 開啟增益時，三組改為增減益精修：除波峰外，也評估有多位置寬頻支持的低處；分開設定單段增益、合成增益、前級能力及額外餘裕。深窄低處或缺少支持的區域不自動當成可增益需求。前級衰減不進入校正目標，避免把整體變小聲算成削峰改善。
- 每個策略有獨立且不變的識別碼。匯出、套用、補錄與延伸都綁定該策略；只切換標籤不會改變目前已套用紀錄。刪除／還原以整個版本為單位；手動修改會另存一個獨立新版本，原三組保留。
- 舊版參數與歷史照常保留。舊 v0.3 的比較紀錄仍是舊政策，不會因安裝新版自動變成三組可套用方案。整組重新計算完全相同時，會選取既有版本，不重複增加版本。

可在「計算依據」查看實際主目標、各策略容許量、每輪搜尋與最終採用路徑。各 Band 的「依據」顯示移除該段後的變化；此比較不重新最佳化，也不等同實測收益。完整模型定義見 [MODEL.md](MODEL.md)，驗收見 [VALIDATION.md](VALIDATION.md)。
...
舊方案不會自動改寫；v0.5 新計算採用 `roompilot-peq-5.0`。手動調整會按該方案保存的限制重新驗算並另存，請查看保存政策與理由。低頻參考與中頻局部趨勢不同時，單一平坦目標仍有局限；不能為了新增一段就無意間降低整個目標。
...
| 設定 | 如何選擇 |
|---|---|
| Band 數 | 本次可分配的數量，範圍 1–128；不必用滿，依實際 DSP 能力填寫 |
...
按「產生 PEQ」後自動新增一個版本，內含三種策略。先選版本，再點策略標籤查看完整曲線及各段依據；匯出、記錄套用與補錄會使用目前策略。如果要延續這一組再擴到 500 Hz，先選取該版的策略，再勾「沿用已選 PEQ，僅擴充頻段」並調整上限。
...
主要模組：`importers.py` 負責被動讀檔；`analysis.py` 負責品質、PEQ 驗算與補錄比較；`solver.py` 執行受限制搜尋；`comparison.py` 建立同一版本的三組策略；`storage.py` 保存本地專案並驗證封存內容；`controller.py` 連接工作佇列與 QML 介面。量測分析使用完整保存資料，圖表另做顯示取樣，不會覆寫來源曲線。
