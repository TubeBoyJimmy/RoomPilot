...
**一般 `.mdat V2` 匯入使用內建解析器，不需要開啟或安裝 REW，也不會在背景啟動 REW。** REW 用來錄製量測；RoomPilot 負責讀取、分析與保存專案。這是第一版實作，支援範圍與尚未完成的驗證請見 [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md)。
...
「標準建議」與「深入搜尋」都使用 CPU、float64 數值分析。深入模式投入更多搜尋，但不保證產生更好的可聽結果；可信的量測、合理的目標與補錄驗證更重要。本版沒有 GPU 計算路徑。
