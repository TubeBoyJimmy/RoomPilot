...
**一般 `.mdat V2` 匯入使用內建解析器，不需要開啟或安裝 REW，也不會在背景啟動 REW。** REW 用來錄製量測；RoomPilot 負責讀取、分析與保存專案。v0.3 的支援範圍與驗證限制請見 [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md)。

## v0.3：看懂取捨，再選定方案

- 按「產生候選」比較最多三組相同 Baseline、目標與能力限制的完整 PEQ。產生及預覽不會增加 PEQ 版本；按「儲存此方案」才保存選定的一版及當時的比較記錄。
- 主指標為「殘留波峰」和「低處額外減益」。前者在全部校正頻點計算高於目標部分的 RMS；後者只在 Baseline 原本低於目標的頻點計算額外減益 RMS。兩者分母不同，不能相加或當成聽感總分。
- 額外減益容許量預設 1 dB RMS，可填 0–6；這是比較門檻，並非聲學標準。每個已量位置都需符合；主畫面顯示各位置 RMS 最大值。**不是每個頻點最多削 1 dB**，也不是只限制新增 Band：保留的 Band 一起計入完整合成 EQ，不含 Preamp。
- 候選內部用不同「削過頭」懲罰強度探索取捨，不以不同政策的總成本互相排名。相同參數會合併；「保護低處／較多削峰／削峰優先」只描述本輪實際比較，非全域最優。結果不符合名稱順序時改用中性名稱。超過容許量的候選可預覽，需調整設定重新比較後才能保存。
- 各 Band 的「依據」顯示保留或移除它時，波峰、低處減益及本策略成本如何改變，並標出 Q／Gain 邊界與保留參數。這是其餘參數固定的反事實比較；搜尋的新增／停止紀錄另外保存，不混稱歷史決策。
- 原先 RMSE 移到次要說明，明示為對平坦目標的誤差，與搜尋成本不同。可展開每個聲道／位置的指標、目標來源與計算依據。
- 舊版可補充只讀說明，標示「按目前規則事後評估」，原參數不改寫。手動修改會重新驗算，原候選排名不會繼承到修改後的方案。
- 候選預覽在關閉、取消重算或切换專案／Baseline 後清除。已選定保存的比較資訊可隨版本查看，也包含在專案包中。
...
舊方案不會自動改寫；v0.3 新計算採用 `roompilot-peq-3.0`。手動調整舊方案會按新版驗算並另存，請查看新版本的限制與理由。低頻參考與中頻局部趨勢不同時，單一平坦目標仍有局限；不能為了新增一段就無意間降低整個目標。
...
按「產生候選」後，在右側比較殘留波峰與低處額外減益，點「預覽」查看曲線及各段依據。選定符合容許量的候選，再按「儲存此方案」。只有保存後的版本能編輯、匯出、記錄套用及補錄。如果要保留這一版再擴到 500 Hz，先保存並選取該版，再使用「保留原案，擴充高頻」。
